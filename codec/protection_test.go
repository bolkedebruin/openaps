...
func TestEncodeSetProtection_SingleFrame(t *testing.T) {
	cases := []struct {
		...
		// Reconnect-frequency trips (BP/BQ): main.exe set_paraName_paraValue_inverter @ 0x69bdc.
		// DS3 sub 0x24 (BP) / 0x23 (BQ), int(Hz × 100), 16-bit.
		{"DS3 BP 49.3", ModelDS3, "Reconnection_under_frequency", 49.3,
			[]byte{0xFB, 0xFB, 0x06, 0xAA, 0x24, 0x00, 0x00, 0x13, 0x42, 0x01, 0x29, 0xFE, 0xFE}},
		{"DS3 BQ 50.5", ModelDS3, "Reconnection_over_frequency", 50.5,
			[]byte{0xFB, 0xFB, 0x06, 0xAA, 0x23, 0x00, 0x00, 0x13, 0xBA, 0x01, 0xA0, 0xFE, 0xFE}},
		// QS1A sub 0x51 (BP) / 0x4E (BQ), int(50e6 / Hz), 24-bit, 0x1C builder.
		{"QS1A BP 49.3", ModelQS1A, "Reconnection_under_frequency", 49.3,
			[]byte{0xFB, 0xFB, 0x06, 0x1C, 0x51, 0x03, 0x0F, 0x79, 0xB6, 0x01, 0xB4, 0xFE, 0xFE}},
		{"QS1A BQ 50.5", ModelQS1A, "Reconnection_over_frequency", 50.5,
			[]byte{0xFB, 0xFB, 0x06, 0x1C, 0x4E, 0x03, 0x0F, 0x1B, 0x93, 0x01, 0x30, 0xFE, 0xFE}},
	}
