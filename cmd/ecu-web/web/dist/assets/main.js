...
Please use the static 'html' tag function. See https://lit.dev/docs/templates/expressions/#static-expressions`);return{["_$litType$"]:X,strings:Z,values:K}},B=CX(dZ),KX=CX(t),Z5=CX(e),x=Symbol.for("lit-noChange"),G=Symbol.for("lit-nothing"),dX=new WeakMap,y=b.createTreeWalker(b,129),E=ZX;function pX(X,Z){if(!OX(X)||!X.hasOwnProperty("raw")){let K="invalid template strings array";throw K=`
        ...
        `.trim().replace(/\n */g,`
`),Error(K)}return EX!==void 0?EX.createHTML(Z):Z}var oZ=(X,Z)=>{let K=X.length-1,Y=[],Q=Z===t?"<svg>":Z===e?"<math>":"",$,j=c;for(let q=0;q<K;q++){let _=X[q],U=-1,k,R=0,W;while(R<_.length){if(j.lastIndex=R,W=j.exec(_),W===null)break;if(R=j.lastIndex,j===c){if(W[fX]==="!--")j=wX;else if(W[fX]!==void 0)j=hX;else if(W[kX]!==void 0){if(vX.test(W[kX]))$=new RegExp(`</${W[kX]}`,"g");j=S}else if(W[hZ]!==void 0)throw Error("Bindings in tag names are not supported. Please use static templates instead. See https://lit.dev/docs/templates/expressions/#static-expressions")}else if(j===S)if(W[gZ]===">")j=$??c,U=-1;else if(W[gX]===void 0)U=-2;else U=j.lastIndex-W[cZ].length,k=W[gX],j=W[cX]===void 0?S:W[cX]==='"'?WX:MX;else if(j===WX||j===MX)j=S;else if(j===wX||j===hX)j=c;else j=S,$=void 0}console.assert(U===-1||j===S||j===MX||j===WX,"unexpected parse state B");let P=j===S&&X[q+1].startsWith("/>")?" ":"";Q+=j===c?_+xZ:U>=0?(Y.push(k),_.slice(0,U)+mX+_.slice(U))+V+P:_+V+(U===-2?q:P)}let H=Q+(X[K]||"<?>")+(Z===t?"</svg>":Z===e?"</math>":"");return[pX(X,H),Y]};class p{constructor({strings:X,["_$litType$"]:Z},K){this.parts=[];let Y,Q=0,$=0,j=X.length-1,H=this.parts,[q,_]=oZ(X,Z);if(this.el=p.createElement(q,K),y.currentNode=this.el.content,Z===t||Z===e){let U=this.el.content.firstChild;U.replaceWith(...U.childNodes)}while((Y=y.nextNode())!==null&&H.length<j){if(Y.nodeType===1){{let U=Y.localName;if(/^(?:textarea|template)$/i.test(U)&&Y.innerHTML.includes(V)){let k=`Expressions are not supported inside \`${U}\` elements. See https://lit.dev/msg/expression-in-${U} for more information.`;if(U==="template")throw Error(k);else m("",k)}}if(Y.hasAttributes()){for(let U of Y.getAttributeNames())if(U.endsWith(mX)){let k=_[$++],W=Y.getAttribute(U).split(V),P=/([.?@])?(.*)/.exec(k);H.push({type:AX,index:Q,name:P[2],strings:W,ctor:P[1]==="."?lX:P[1]==="?"?sX:P[1]==="@"?iX:l}),Y.removeAttribute(U)}else if(U.startsWith(V))H.push({type:IX,index:Q}),Y.removeAttribute(U)}if(vX.test(Y.tagName)){let U=Y.textContent.split(V),k=U.length-1;if(k>0){Y.textContent=n?n.emptyScript:"";for(let R=0;R<k;R++)Y.append(U[R],u()),y.nextNode(),H.push({type:XX,index:++Q});Y.append(U[k],u())}}}else if(Y.nodeType===8)if(Y.data===uX)H.push({type:XX,index:Q});else{let k=-1;while((k=Y.data.indexOf(V,k+1))!==-1)H.push({type:pZ,index:Q}),k+=V.length-1}Q++}if(_.length!==$)throw Error('Detected duplicate attribute bindings. This occurs if your template has duplicate attributes on an element tag. For example "<input ?disabled=${true} ?disabled=${false}>" contains a duplicate "disabled" attribute. The error was detected in the following template: \n`'+X.join("${...}")+"`");F&&F({kind:"template prep",template:this,clonableTemplate:this.el,parts:this.parts,strings:X})}static createElement(X,Z){let K=b.createElement("template");return K.innerHTML=X,K}}function h(X,Z,K=X,Y){if(Z===x)return Z;let Q=Y!==void 0?K.__directives?.[Y]:K.__directive,$=v(Z)?void 0:Z._$litDirective$;if(Q?.constructor!==$){if(Q?._$notifyDirectiveConnectionChanged?.(!1),$===void 0)Q=void 0;else Q=new $(X),Q._$initialize(X,K,Y);if(Y!==void 0)(K.__directives??=[])[Y]=Q;else K.__directive=Q}if(Q!==void 0)Z=h(X,Q._$resolve(X,Z.values),Q,Y);return Z}class oX{constructor(X,Z){this._$parts=[],this._$disconnectableChildren=void 0,this._$template=X,this._$parent=Z}get parentNode(){return this._$parent.parentNode}get _$isConnected(){return this._$parent._$isConnected}_clone(X){let{el:{content:Z},parts:K}=this._$template,Y=(X?.creationScope??b).importNode(Z,!0);y.currentNode=Y;let Q=y.nextNode(),$=0,j=0,H=K[0];while(H!==void 0){if($===H.index){let q;if(H.type===XX)q=new o(Q,Q.nextSibling,this,X);else if(H.type===AX)q=new H.ctor(Q,H.name,H.strings,this,X);else if(H.type===IX)q=new rX(Q,this,X);this._$parts.push(q),H=K[++j]}if($!==H?.index)Q=y.nextNode(),$++}return y.currentNode=b,Y}_update(X){let Z=0;for(let K of this._$parts){if(K!==void 0)if(F&&F({kind:"set part",part:K,value:X[Z],valueIndex:Z,values:X,templateInstance:this}),K.strings!==void 0)K._$setValue(X,K,Z),Z+=K.strings.length-2;else K._$setValue(X[Z]);Z++}}}class o{get _$isConnected(){return this._$parent?._$isConnected??this.__isConnected}constructor(X,Z,K,Y){this.type=XX,this._$committedValue=G,this._$disconnectableChildren=void 0,this._$startNode=X,this._$endNode=Z,this._$parent=K,this.options=Y,this.__isConnected=Y?.isConnected??!0,this._textSanitizer=void 0}get parentNode(){let X=N(this._$startNode).parentNode,Z=this._$parent;if(Z!==void 0&&X?.nodeType===11)X=Z.parentNode;return X}get startNode(){return this._$startNode}get endNode(){return this._$endNode}_$setValue(X,Z=this){if(this.parentNode===null)throw Error("This `ChildPart` has no `parentNode` and therefore cannot accept a value. This likely means the element containing the part was manipulated in an unsupported way outside of Lit's control such that the part's marker nodes were ejected from DOM. For example, setting the element's `innerHTML` or `textContent` can do this.");if(X=h(this,X,Z),v(X)){if(X===G||X==null||X===""){if(this._$committedValue!==G)F&&F({kind:"commit nothing to child",start:this._$startNode,end:this._$endNode,parent:this._$parent,options:this.options}),this._$clear();this._$committedValue=G}else if(X!==this._$committedValue&&X!==x)this._commitText(X)}else if(X._$litType$!==void 0)this._commitTemplateResult(X);else if(X.nodeType!==void 0){if(this.options?.host===X){this._commitText("[probable mistake: rendered a template's host in itself (commonly caused by writing ${this} in a template]"),console.warn("Attempted to render the template host",X,"inside itself. This is almost always a mistake, and in dev mode ","we render some warning text. In production however, we'll ","render it, which will usually result in an error, and sometimes ","in the element disappearing from the DOM.");return}this._commitNode(X)}else if(EZ(X))this._commitIterable(X);else this._commitText(X)}_insert(X){return N(N(this._$startNode).parentNode).insertBefore(X,this._$endNode)}_commitNode(X){if(this._$committedValue!==X){if(this._$clear(),E!==ZX){let Z=this._$startNode.parentNode?.nodeName;if(Z==="STYLE"||Z==="SCRIPT"){let K="Forbidden";if(Z==="STYLE")K="Lit does not support binding inside style nodes. This is a security risk, as style injection attacks can exfiltrate data and spoof UIs. Consider instead using css`...` literals to compose styles, and do dynamic styling with css custom properties, ::parts, <slot>s, and by mutating the DOM rather than stylesheets.";else K="Lit does not support binding inside script nodes. This is a security risk, as it could allow arbitrary code execution.";throw Error(K)}}F&&F({kind:"commit node",start:this._$startNode,parent:this._$parent,value:X,options:this.options}),this._$committedValue=this._insert(X)}}_commitText(X){if(this._$committedValue!==G&&v(this._$committedValue)){let Z=N(this._$startNode).nextSibling;if(this._textSanitizer===void 0)this._textSanitizer=_X(Z,"data","property");X=this._textSanitizer(X),F&&F({kind:"commit text",node:Z,value:X,options:this.options}),Z.data=X}else{let Z=b.createTextNode("");if(this._commitNode(Z),this._textSanitizer===void 0)this._textSanitizer=_X(Z,"data","property");X=this._textSanitizer(X),F&&F({kind:"commit text",node:Z,value:X,options:this.options}),Z.data=X}this._$committedValue=X}_commitTemplateResult(X){let{values:Z,["_$litType$"]:K}=X,Y=typeof K==="number"?this._$getTemplate(X):(K.el===void 0&&(K.el=p.createElement(pX(K.h,K.h[0]),this.options)),K);if(this._$committedValue?._$template===Y)F&&F({kind:"template updating",template:Y,instance:this._$committedValue,parts:this._$committedValue._$parts,options:this.options,values:Z}),this._$committedValue._update(Z);else{let Q=new oX(Y,this),$=Q._clone(this.options);F&&F({kind:"template instantiated",template:Y,instance:Q,parts:Q._$parts,options:this.options,fragment:$,values:Z}),Q._update(Z),F&&F({kind:"template instantiated and updated",template:Y,instance:Q,parts:Q._$parts,options:this.options,fragment:$,values:Z}),this._commitNode($),this._$committedValue=Q}}_$getTemplate(X){let Z=dX.get(X.strings);if(Z===void 0)dX.set(X.strings,Z=new p(X));return Z}_commitIterable(X){if(!OX(this._$committedValue))this._$committedValue=[],this._$clear();let Z=this._$committedValue,K=0,Y;for(let Q of X){if(K===Z.length)Z.push(Y=new o(this._insert(u()),this._insert(u()),this,this.options));else Y=Z[K];Y._$setValue(Q),K++}if(K<Z.length)this._$clear(Y&&N(Y._$endNode).nextSibling,K),Z.length=K}_$clear(X=N(this._$startNode).nextSibling,Z){this._$notifyConnectionChanged?.(!1,!0,Z);while(X!==this._$endNode){let K=N(X).nextSibling;N(X).remove(),X=K}}setConnected(X){if(this._$parent===void 0)this.__isConnected=X,this._$notifyConnectionChanged?.(X);else throw Error("part.setConnected() may only be called on a RootPart returned from render().")}}class l{get tagName(){return this.element.tagName}get _$isConnected(){return this._$parent._$isConnected}constructor(X,Z,K,Y,Q){if(this.type=AX,this._$committedValue=G,this._$disconnectableChildren=void 0,this.element=X,this.name=Z,this._$parent=Y,this.options=Q,K.length>2||K[0]!==""||K[1]!=="")this._$committedValue=Array(K.length-1).fill(new String),this.strings=K;else this._$committedValue=G;this._sanitizer=void 0}_$setValue(X,Z=this,K,Y){let Q=this.strings,$=!1;if(Q===void 0){if(X=h(this,X,Z,0),$=!v(X)||X!==this._$committedValue&&X!==x,$)this._$committedValue=X}else{let j=X;X=Q[0];let H,q;for(H=0;H<Q.length-1;H++){if(q=h(this,j[K+H],Z,H),q===x)q=this._$committedValue[H];if($||=!v(q)||q!==this._$committedValue[H],q===G)X=G;else if(X!==G)X+=(q??"")+Q[H+1];this._$committedValue[H]=q}}if($&&!Y)this._commitValue(X)}_commitValue(X){if(X===G)N(this.element).removeAttribute(this.name);else{if(this._sanitizer===void 0)this._sanitizer=E(this.element,this.name,"attribute");X=this._sanitizer(X??""),F&&F({kind:"commit attribute",element:this.element,name:this.name,value:X,options:this.options}),N(this.element).setAttribute(this.name,X??"")}}}class lX extends l{constructor(){super(...arguments);this.type=mZ}_commitValue(X){if(this._sanitizer===void 0)this._sanitizer=E(this.element,this.name,"property");X=this._sanitizer(X),F&&F({kind:"commit property",element:this.element,name:this.name,value:X,options:this.options}),this.element[this.name]=X===G?void 0:X}}class sX extends l{constructor(){super(...arguments);this.type=uZ}_commitValue(X){F&&F({kind:"commit boolean attribute",element:this.element,name:this.name,value:!!(X&&X!==G),options:this.options}),N(this.element).toggleAttribute(this.name,!!X&&X!==G)}}class iX extends l{constructor(X,Z,K,Y,Q){super(X,Z,K,Y,Q);if(this.type=vZ,this.strings!==void 0)throw Error(`A \`<${X.localName}>\` has a \`@${Z}=...\` listener with invalid content. Event listeners in templates must have exactly one expression and no surrounding text.`)}_$setValue(X,Z=this){if(X=h(this,X,Z,0)??G,X===x)return;let K=this._$committedValue,Y=X===G&&K!==G||X.capture!==K.capture||X.once!==K.once||X.passive!==K.passive,Q=X!==G&&(K===G||Y);if(F&&F({kind:"commit event listener",element:this.element,name:this.name,value:X,options:this.options,removeListener:Y,addListener:Q,oldListener:K}),Y)this.element.removeEventListener(this.name,this,K);if(Q)this.element.addEventListener(this.name,this,X);this._$committedValue=X}handleEvent(X){if(typeof this._$committedValue==="function")this._$committedValue.call(this.options?.host??this.element,X);else this._$committedValue.handleEvent(X)}}class rX{constructor(X,Z,K){this.element=X,this.type=IX,this._$disconnectableChildren=void 0,this._$parent=Z,this.options=K}get _$isConnected(){return this._$parent._$isConnected}_$setValue(X){F&&F({kind:"commit to element binding",element:this.element,value:X,options:this.options}),h(this,X)}}var lZ=C.litHtmlPolyfillSupportDevMode;lZ?.(p,o);(C.litHtmlVersions??=[]).push("3.3.3");if(C.litHtmlVersions.length>1)queueMicrotask(()=>{m("multiple-versions","Multiple versions of Lit loaded. Loading multiple versions is not recommended.")});var d=(X,Z,K)=>{if(Z==null)throw TypeError(`The container to render into may not be ${Z}`);let Y=SZ++,Q=K?.renderBefore??Z,$=Q._$litPart$;if(F&&F({kind:"begin render",id:Y,value:X,container:Z,options:K,part:$}),$===void 0){let j=K?.renderBefore??null;Q._$litPart$=$=new o(Z.insertBefore(u(),j),j,void 0,K??{})}return $._$setValue(X),F&&F({kind:"end render",id:Y,value:X,container:Z,options:K,part:$}),$};d.setSanitizer=yZ,d.createSanitizer=_X,d._testOnlyClearSanitizerFactoryDoNotCallOrElse=bZ;var sZ=(X,Z)=>X,TX=!0,L=globalThis,aX;if(TX)L.litIssuedWarnings??=new Set,aX=(X,Z)=>{if(Z+=` See https://lit.dev/msg/${X} for more information.`,!L.litIssuedWarnings.has(Z)&&!L.litIssuedWarnings.has(X))console.warn(Z),L.litIssuedWarnings.add(Z)};class z extends I{constructor(){super(...arguments);this.renderOptions={host:this},this.__childPart=void 0}createRenderRoot(){let X=super.createRenderRoot();return this.renderOptions.renderBefore??=X.firstChild,X}update(X){let Z=this.render();if(!this.hasUpdated)this.renderOptions.isConnected=this.isConnected;super.update(X),this.__childPart=d(Z,this.renderRoot,this.renderOptions)}connectedCallback(){super.connectedCallback(),this.__childPart?.setConnected(!0)}disconnectedCallback(){super.disconnectedCallback(),this.__childPart?.setConnected(!1)}render(){return x}}z._$litElement$=!0;z[sZ("finalized",z)]=!0;L.litElementHydrateSupport?.({LitElement:z});var iZ=TX?L.litElementPolyfillSupportDevMode:L.litElementPolyfillSupport;iZ?.({LitElement:z});(L.litElementVersions??=[]).push("4.2.2");if(TX&&L.litElementVersions.length>1)queueMicrotask(()=>{aX("multiple-versions","Multiple versions of Lit loaded. Loading multiple versions is not recommended.")});async function f(X){let Z=await fetch(X,{credentials:"same-origin"});if(!Z.ok)throw Error(`${X}: ${Z.status}`);return await Z.json()}async function YX(X,Z){let K=await fetch(X,{method:"POST",credentials:"same-origin",headers:{"Content-Type":"application/json"},body:JSON.stringify(Z)});if(!K.ok){let Y=await K.text();throw Error(Y.trim()||`${X}: ${K.status}`)}}async function nX(X,Z){let K=await fetch(X,{method:"PUT",credentials:"same-origin",headers:{"Content-Type":"application/json"},body:JSON.stringify(Z)});if(!K.ok){let Y=await K.text();throw Error(Y.trim()||`${X}: ${K.status}`)}return await K.json()}async function rZ(X,Z){let K=await fetch(X,{method:"DELETE",credentials:"same-origin",headers:{"Content-Type":"application/json"},body:JSON.stringify(Z)});if(!K.ok){let Y=await K.text();throw Error(Y.trim()||`${X}: ${K.status}`)}return await K.json()}var M={authStatus:()=>f("/api/auth/status"),setup:(X)=>YX("/api/auth/setup",{password:X}),login:(X)=>YX("/api/auth/login",{password:X}),logout:()=>YX("/api/auth/logout",{}),fleet:()=>f("/api/fleet"),system:()=>f("/api/system"),history:()=>f("/api/history"),events:(X={})=>{let Z=new URLSearchParams;if(X.since_ms)Z.set("since_ms",String(X.since_ms));if(X.kind)Z.set("kind",X.kind);if(X.severity)Z.set("severity",X.severity);if(X.inverter_uid)Z.set("inverter_uid",X.inverter_uid);if(X.limit)Z.set("limit",String(X.limit));let K=Z.toString();return f("/api/events"+(K?`?${K}`:""))},getSettings:async()=>{let X=await f("/api/settings");if(X.error)return{error:X.error};return{settings:{ecu_id:X.ecu_id,mac:X.mac,pan_override:X.pan_override,zigbee_type:X.zigbee_type,inverter_names:X.inverter_names??{}}}},saveSettings:(X)=>nX("/api/settings",X),profiles:()=>f("/api/profiles"),selectBase:(X)=>YX("/api/profiles/base",{id:X}),saveOverlay:(X)=>nX("/api/profiles/overlay",X),deleteOverlay:(X,Z)=>rZ("/api/profiles/overlay",{id:X,uids:Z})};function tX(X,Z){let K=new EventSource("/api/stream");return K.addEventListener("fleet",(Y)=>{try{X(JSON.parse(Y.data))}catch{}}),K.onerror=()=>Z?.(),()=>K.close()}class eX extends z{static properties={configured:{type:Boolean},error:{state:!0},busy:{state:!0}};constructor(){super();this.configured=!0,this.error="",this.busy=!1}static styles=J`
    :host {
      display: grid;
      place-items: center;
      min-height: 100vh;
  ...
  `;pct(){if(!(this.cap>0))return 0;return Math.max(0,Math.min(100,this.power/this.cap*100))}render(){let X=this.pct(),Z=BX(X),K=90,Y=Math.PI*90,Q=Y*(1-X/100);return B`
    ...
    `}}customElements.define("fleet-gauge",YZ);class QZ extends z{static properties={label:{type:String},value:{type:String},sub:{type:String}};constructor(){super();this.label="",this.value="",this.sub=""}static styles=J`
    :host {
      display: block;
      background: var(--surface);
      border: 1px solid var(--border);
  ...
  `;render(){return B`
    ...
    `}}customElements.define("stat-card",QZ);class BZ extends z{static properties={inverter:{attribute:!1},name:{type:String}};constructor(){super();this.name=""}static styles=J`
    :host {
      display: block;
      background: var(--surface);
      border: 1px solid var(--border);
  ...
  `;render(){let X=this.inverter;if(!X)return G;let Z=BX(X.load_pct),K=$X(X.faults),Y=Math.max(0,Math.min(100,X.load_pct));return B`
    ...
    `}}customElements.define("inverter-card",BZ);class $Z extends z{static properties={system:{attribute:!1}};constructor(){super();this.system=null}static styles=J`
    :host { display: block; }
    .id {
      display: grid;
  ...
  `;onMove=(X)=>{let Z=this.points.length;if(Z<2)return;let Y=X.currentTarget.clientWidth||1,Q=Math.min(1,Math.max(0,X.offsetX/Y));this.hoverIdx=Math.round(Q*(Z-1))};onLeave=()=>{this.hoverIdx=-1};render(){let X=this.points??[];if(X.length<2)return B`<div class="empty">Collecting power history…</div>`;let{line:Z,area:K,max:Y}=aZ(X,GX,r),Q=X[X.length-1].w,$=this.hoverIdx,j=$>=0&&$<X.length,H=X[0].t,q=Math.max(1,X[X.length-1].t-H),_=j?(X[$].t-H)/q*GX:0,U=j?r-X[$].w/Y*r:0;return B`
      <div class="wrap">
        ...
        >
          ...
          ${j?KX`<line class="cross" x1=${_} y1="0" x2=${_} y2=${r} /><circle class="cursor" cx=${_} cy=${U} r="3.5" />`:G}
        </svg>
        ${j?B`<div class="tip" style="left:${_/GX*100}%; top:${U}px">
    ...
    `}}customElements.define("power-chart",jZ);class GZ extends z{static properties={fleet:{attribute:!1},system:{attribute:!1},names:{attribute:!1},history:{state:!0}};timer=null;constructor(){super();this.fleet=null,this.system=null,this.names={},this.history=[]}connectedCallback(){super.connectedCallback(),this.loadHistory(),this.timer=setInterval(()=>void this.loadHistory(),60000)}disconnectedCallback(){if(super.disconnectedCallback(),this.timer)clearInterval(this.timer);this.timer=null}async loadHistory(){try{this.history=await M.history()}catch{}}chartPoints(){if(!this.fleet)return this.history;return[...this.history,{t:Date.now(),w:this.fleet.active_power_w}]}static styles=J`
    :host { display: block; }
    .grid {
      display: grid;
  ...
  `;render(){let X=this.fleet;if(!X)return B`<div class="empty">Waiting for inv-driver…</div>`;return B`
      ...
      ${X.inverters.length?B`<div class="cards">
            ${X.inverters.map((Z)=>B`<inverter-card .inverter=${Z} .name=${this.names?.[Z.uid]??""}></inverter-card>`)}
          </div>`:B`<div class="empty">No inverters discovered yet.</div>`}
      ${G}
    `}}customElements.define("dashboard-view",GZ);class zZ extends z{static properties={fleet:{attribute:!1},names:{attribute:!1}};constructor(){super();this.fleet=null,this.names={}}rename(X,Z){let K=Z.target.value;this.dispatchEvent(new CustomEvent("rename",{detail:{uid:X,name:K},bubbles:!0,composed:!0}))}static styles=J`
    :host { display: block; }
  ...
  `;render(){let X=this.fleet;if(!X||X.inverters.length===0)return B`<div class="empty">No inverters discovered yet.</div>`;return B`
    ...
    `}}customElements.define("inverters-view",zZ);class HZ extends z{static properties={fleet:{attribute:!1}};constructor(){super();this.fleet=null}static styles=J`
    :host { display: block; }
    .row {
      display: flex;
      align-items: center;
  ...
  `;alarms(){let X=[];for(let Z of this.fleet?.inverters??[]){for(let K of $X(Z.faults))X.push({uid:Z.uid,model:Z.model,label:K,severity:"fault"});if(!Z.online)X.push({uid:Z.uid,model:Z.model,label:"Inverter offline",severity:"warning"})}return X}render(){let X=this.alarms();if(X.length===0)return B`<div class="ok"><div class="big">✓ No active alarms</div><div>All inverters reporting healthy.</div></div>`;return B`${X.map((Z)=>B`<div class="row ${Z.severity}">
      ...
      </div>`)}`}}customElements.define("alarms-view",HZ);class JZ extends z{static properties={events:{attribute:!1}};constructor(){super();this.events=[]}static styles=J`
    :host { display: block; }
    table { width: 100%; border-collapse: collapse; font-size: 13px; }
  ...
  `;render(){if(!this.events||this.events.length===0)return B`<div class="empty">No events recorded.</div>`;return B`
    ...
    `}}customElements.define("events-table",JZ);class UZ extends z{static properties={events:{state:!0},error:{state:!0},loading:{state:!0}};timer=null;constructor(){super();this.events=[],this.error="",this.loading=!1}static styles=J`
    :host { display: block; }
  ...
  `;connectedCallback(){super.connectedCallback(),this.load(),this.timer=setInterval(()=>void this.load(),15000)}disconnectedCallback(){if(super.disconnectedCallback(),this.timer)clearInterval(this.timer);this.timer=null}async load(){this.loading=!0;try{let X=await M.events({limit:200});this.events=X.events??[],this.error=X.error??""}catch(X){this.error=X.message}finally{this.loading=!1}}render(){return B`
    ...
    `}}customElements.define("events-view",UZ);class FZ extends z{static properties={profiles:{attribute:!1},activeBase:{attribute:!1},reconcilerReady:{attribute:!1},busy:{attribute:!1},selected:{state:!0}};constructor(){super();this.profiles=[],this.activeBase="",this.reconcilerReady=!0,this.busy=!1,this.selected=""}static styles=J`
    :host { display: block; }
  ...
  `;onChange=(X)=>{this.selected=X.target.value};apply=()=>{let X=this.effectiveSelected();if(!X||X===this.activeBase)return;this.dispatchEvent(new CustomEvent("apply",{detail:X,bubbles:!0,composed:!0}))};effectiveSelected(){return this.selected||this.activeBase}labelFor(X){let Z=[`${X.vnom_v} V`];if(X.source_ref)Z.push(X.source_ref);return Z.push(`${X.point_count} pts`),`${X.id} — ${Z.join(" · ")}`}render(){let X=this.effectiveSelected(),Z=this.profiles.find((Y)=>Y.id===this.activeBase),K=!this.busy&&this.reconcilerReady&&X!==""&&X!==this.activeBase;return B`
    ...
    `}}customElements.define("grid-profile-form",FZ);class qZ extends z{static properties={params:{attribute:!1},inverters:{attribute:!1},profile:{attribute:!1},names:{attribute:!1},busy:{attribute:!1},editing:{attribute:!1},name:{state:!0},selectedUids:{state:!0},values:{state:!0},localError:{state:!0}};constructor(){super();this.params=[],this.inverters=[],this.profile=null,this.names={},this.busy=!1,this.editing=!1,this.name="",this.selectedUids=[],this.values={},this.localError=""}static styles=J`
    :host { display: block; }
  ...
  `;willUpdate(X){if(X.has("profile")){let Z=this.profile;this.name=Z?.id??"",this.selectedUids=[...Z?.uids??[]];let K={};for(let Y of Z?.points??[])K[Y.aps_code]=String(Y.value);this.values=K,this.localError=""}}effectiveWritable(){if(!this.selectedUids.length)return new Set;let X=this.selectedUids.map((K)=>new Set(this.inverters.find((Y)=>Y.uid===K)?.writable_codes??[])),Z=X[0];for(let K of X.slice(1))Z=new Set([...Z].filter((Y)=>K.has(Y)));return Z}label(X){return this.names[X.uid]||X.model||X.uid}toggleTarget(X,Z){this.selectedUids=Z?[...this.selectedUids,X]:this.selectedUids.filter((K)=>K!==X)}setValue(X,Z){this.values={...this.values,[X]:Z}}save=()=>{let X=this.effectiveWritable(),Z=this.params.filter((Y)=>X.has(Y.aps_code)).map((Y)=>({p:Y,raw:(this.values[Y.aps_code]??"").trim()})).filter((Y)=>Y.raw!==""&&!Number.isNaN(Number(Y.raw))).map((Y)=>({aps_code:Y.p.aps_code,value:Number(Y.raw)}));if(!this.name.trim())return void(this.localError="Profile name is required.");if(!this.selectedUids.length)return void(this.localError="Select at least one target inverter.");if(!Z.length)return void(this.localError="Set at least one parameter value.");this.localError="";let K={id:this.name.trim(),uids:this.selectedUids,points:Z};this.dispatchEvent(new CustomEvent("save",{detail:K,bubbles:!0,composed:!0}))};cancel=()=>{this.dispatchEvent(new CustomEvent("cancel",{bubbles:!0,composed:!0}))};render(){let X=this.effectiveWritable(),Z=this.selectedUids.length>0;return B`
    ...
    `}}customElements.define("local-site-profile-form",qZ);class kZ extends z{static properties={data:{state:!0},names:{state:!0},error:{state:!0},notice:{state:!0},baseBusy:{state:!0},overlayBusy:{state:!0},editing:{state:!0},editingExisting:{state:!0}};constructor(){super();this.data=null,this.names={},this.error="",this.notice="",this.baseBusy=!1,this.overlayBusy=!1,this.editing=null,this.editingExisting=!1}static styles=J`
    :host { display: block; }
    .panel {
      background: var(--surface);
      border: 1px solid var(--border);
    ...
    .empty { color: var(--muted); font-size: 13px; }
  `;connectedCallback(){super.connectedCallback(),this.load()}async load(){try{let[X,Z]=await Promise.all([M.profiles(),M.getSettings()]);this.data=X,this.error=X.error??"",this.names=Z.settings?.inverter_names??{}}catch(X){this.error=X.message}}invName(X){if(this.names[X])return this.names[X];return this.data?.inverters.find((K)=>K.uid===X)?.model||X}onSelectBase=async(X)=>{let Z=X.detail;if(!window.confirm(`Apply base grid profile "${Z}" to every inverter? This writes grid-protection settings across the whole fleet.`))return;this.baseBusy=!0,this.notice="",this.error="";try{await M.selectBase(Z),this.notice=`Base profile "${Z}" applied.`}catch(K){this.error=K.message}finally{this.baseBusy=!1,await this.load()}};newProfile(){this.editing={id:"",uids:[],points:[]},this.editingExisting=!1,this.notice="",this.error=""}editProfile(X){this.editing=X,this.editingExisting=!0,this.notice="",this.error=""}onCancelEdit=()=>{this.editing=null};onSaveOverlay=async(X)=>{let Z=X.detail;if(!window.confirm(`Apply Local Site profile "${Z.id}" to ${Z.uids.length} inverter(s)? This writes grid-protection parameters to each.`))return;this.overlayBusy=!0,this.notice="",this.error="";try{let K=await M.saveOverlay(Z);this.reportResults(Z.id,K.results),this.editing=null}catch(K){this.error=K.message}finally{this.overlayBusy=!1,await this.load()}};deleteProfile=async(X)=>{if(!window.confirm(`Delete Local Site profile "${X.id}" and clear it from ${X.uids.length} inverter(s)?`))return;this.overlayBusy=!0,this.notice="",this.error="";try{let Z=await M.deleteOverlay(X.id,X.uids);if(this.reportResults(X.id,Z.results,"cleared"),this.editing?.id===X.id)this.editing=null}catch(Z){this.error=Z.message}finally{this.overlayBusy=!1,await this.load()}};reportResults(X,Z,K="applied"){let Y=Z.filter((Q)=>!Q.ok);if(Y.length===0)this.notice=`Profile "${X}" ${K} to ${Z.length} inverter(s).`;else this.notice=`Profile "${X}" saved; not confirmed on ${Y.length} of ${Z.length}.`,this.error=Y.map((Q)=>`${this.invName(Q.uid)}: ${Q.error||"unconfirmed"}`).join("; ")}renderBase(){let X=this.data?.base;return B`
      <div class="panel">
    ...
    `}}customElements.define("profiles-view",kZ);class MZ extends z{static properties={settings:{attribute:!1}};constructor(){super();this.settings={ecu_id:"",mac:"",pan_override:"",zigbee_type:"apsystems"}}static styles=J`
    :host { display: block; }
  ...
  `;save=()=>{let X=this.shadowRoot;if(!X)return;let Z=(Y)=>(X.querySelector(`#${Y}`)?.value??"").trim(),K={ecu_id:Z("ecu_id"),mac:Z("mac"),pan_override:Z("pan_override"),zigbee_type:Z("zigbee_type")};this.dispatchEvent(new CustomEvent("save",{detail:K,bubbles:!0,composed:!0}))};render(){let X=this.settings;return B`
    ...
    `}}customElements.define("settings-form",MZ);class WZ extends z{static properties={settings:{state:!0},error:{state:!0},notice:{state:!0},loading:{state:!0},saving:{state:!0}};constructor(){super();this.settings=null,this.error="",this.notice="",this.loading=!1,this.saving=!1}static styles=J`
    :host { display: block; }
    .panel {
      background: var(--surface);
      border: 1px solid var(--border);
  ...
  `;connectedCallback(){super.connectedCallback(),window.addEventListener("hashchange",this.onHash),this.onHash(),this.init()}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("hashchange",this.onHash),this.stopStreams()}onHash=()=>{let X=(location.hash.replace(/^#\/?/,"")||"dashboard").split("/")[0];this.route=NX.some((Z)=>Z.id===X)?X:"dashboard"};async init(){try{let X=await M.authStatus();if(this.configured=X.configured,this.authed=X.authenticated,this.authed)this.startStreams()}catch{}finally{this.ready=!0}}onAuthed=async()=>{this.authed=!0,this.startStreams()};logout=async()=>{try{await M.logout()}catch{}this.authed=!1,this.stopStreams(),this.fleet=null,this.system=null};startStreams(){this.stopStreams(),this.closeSSE=tX((Z)=>{this.fleet=Z});let X=()=>M.system().then((Z)=>this.system=Z).catch(()=>{});X(),this.sysTimer=setInterval(X,5000),this.fetchSettings()}async fetchSettings(){try{let X=await M.getSettings();if(X.settings)this.settingsCache=X.settings,this.names=X.settings.inverter_names??{}}catch{}}onRename=async(X)=>{let{uid:Z,name:K}=X.detail,Y=this.settingsCache??{ecu_id:"",mac:"",pan_override:"",zigbee_type:""},Q={...Y.inverter_names??{}};if(K.trim())Q[Z]=K.trim();else delete Q[Z];let $={...Y,inverter_names:Q};try{await M.saveSettings($),this.settingsCache=$,this.names=Q}catch{}};stopStreams(){if(this.closeSSE?.(),this.closeSSE=null,this.sysTimer)clearInterval(this.sysTimer);this.sysTimer=null}activeView(){switch(this.route){case"inverters":return B`<inverters-view
          .fleet=${this.fleet}
          .names=${this.names}
          @rename=${this.onRename}
        ></inverters-view>`;case"alarms":return B`<alarms-view .fleet=${this.fleet}></alarms-view>`;case"events":return B`<events-view></events-view>`;case"profiles":return B`<profiles-view></profiles-view>`;case"settings":return B`<settings-view></settings-view>`;default:return B`<dashboard-view .fleet=${this.fleet} .system=${this.system} .names=${this.names}></dashboard-view>`}}render(){if(!this.ready)return G;if(!this.authed)return B`<login-view .configured=${this.configured} @authed=${this.onAuthed}></login-view>`;let X=NX.find((K)=>K.id===this.route)?.label??"Dashboard",Z=this.system?.invdriver_connected??!1;return B`
      <div class="layout">
        <nav>
